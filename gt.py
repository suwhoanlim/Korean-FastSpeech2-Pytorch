...
def synthesize(model, vocoder, text, sentence, prefix=''):
    ...
    if vocoder is not None:
        if hp.vocoder.lower() == "vocgan":
            mel_gt, _ = Audio.tools.get_mel("/home/test/Korean-FastSpeech2-Pytorch_64d_head2/gt1-4.wav", 1)
            name = 'gt1-4'
            #mel_gt = np.load('/home/test/Korean-FastSpeech2-Pytorch_64d_head2/preprocessed/kss/mel/kss-mel-1_0002.npy')
            #mel_gtt = torch.from_numpy(mel_gt).float().to(device)
            mel_gt = mel_gt.to(device)
            pdb.set_trace()
            mel_gt = mel_gt.unsqueeze(0)
            #pdb.set_trace()
            utils.vocgan_infer(mel_gt, vocoder, path=os.path.join(hp.test_path, 'gt_{}_{}_{}.wav'.format(prefix, hp.vocoder, name)))   
...
if __name__ == "__main__":
    ...
    #test_sentence=['안녕하세요, 한동대학니다.', '이 프로젝트가 여러분에게 도움이 되었으면 좋겠습니다.', '시간이 촉박해요','이런, 큰일 났어','좀 더 먹지 그래?','제가 뭘 잘못했죠?','더 이상 묻지마']
    test_sentence=['좀 더 먹지 그래?','제가 뭘 잘못했죠?','더 이상 묻지마']
    g2p=G2p()
